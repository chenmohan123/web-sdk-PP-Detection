...
- 合并采用第二轮验证的 confident-anchor 策略；用户阈值在合并后应用，结果包含实际推理次数，耗时覆盖全部切片。
- Demo 图片模式新增紧凑开关，进度复用状态行；模型版本、来源和稳定状态保持现有设置。
- 小米 15 局域网 HTTPS 人工反馈“基本上是对的”，未逐项确认模型、精度、后端和浏览器版本；不扩展为全设备兼容承诺。增强仍可能增加误检和耗时，详见[发布说明](docs/zh-CN/release-0.4.0.md)。
...
- 模型清单和资产字节沿用既有发布版本；显式来源失败不静默换源。模型、许可与验证边界见[发布说明](docs/zh-CN/release-0.3.2.md)。
