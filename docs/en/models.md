...
> 2026-09-16: 14 specifications and 39 stable variants. Tiny 0.1.1 adds the qualified FP16 variant; W8A32 remains labs-only. SDK/npm remains 0.4.0.

[中文](../zh-CN/models.md)

## Current stable manifests (2026-09-16)

2026-09-16: 14 specifications and 39 stable variants. Tiny 0.1.1 adds the qualified FP16 variant; W8A32 remains labs-only. Defaults are FP32 and ModelScope, with explicit Hugging Face selection available. Precisions that failed the recognition gate stay out of stable manifests.
...
| PP-YOLO Tiny 320     | 0.1.1   |   4.51 MB |   2.36 MB |        — |
...
- PP-YOLO Tiny manifest: [`models/ppyolo-tiny-320/0.1.1/manifest.json`](../../models/ppyolo-tiny-320/0.1.1/manifest.json)
...
All 39 variants declare WASM and WebGPU support. Runtime capability probing still applies. An explicitly selected unsupported combination returns `CAPABILITY_UNSUPPORTED` without silently replacing the precision. `backend: "auto"` tries another valid backend after a session or inference failure only with `allowFallback: true`.
...
Tiny 0.1.1 reuses the immutable FP32 sources from 0.1.0 and adds the 2.36 MB FP16 variant. FP16 passed the fixed 64-image, three-run gate on both backends with a worst AP change of -0.174649 points and minimum one-to-one detection retention of 99.5192%. W8A32 missed the AP gate at -0.537364 points, remains labs-only, was not uploaded, and is not exposed by the Demo. See the [quality report](../../reports/evaluation/2026-09-16-tiny-precision/README.md) and [distribution verification](../../reports/distribution/2026-09-16-tiny-precision/README.md).
