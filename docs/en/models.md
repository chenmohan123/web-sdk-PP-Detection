...
## Current stable manifests (2026-09-14)
...
All 15 variants declare WASM and WebGPU support. Runtime capability probing still applies. An explicitly selected unsupported combination returns `CAPABILITY_UNSUPPORTED` without silently replacing the precision. `backend: "auto"` tries another valid backend after a session or inference failure only with `allowFallback: true`.
