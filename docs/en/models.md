...
download URLs, byte sizes, and SHA-256 values. The 1.0.1 FP32 candidate has passed seven-fixture
validation on Linux WASM and Windows NVIDIA WebGPU, but FP16, mobile browser, WeChat WebView,
and source licensing verification are still pending, so it is not yet a stable SDK default. Pass a
...
The 1.0.1 FP32 browser reports and workflow index are in `tools/model-pipeline/reports/1.0.1/remote-validation.json`; these results apply only to the browser, operating-system, and NVIDIA device matrix recorded in the reports.
