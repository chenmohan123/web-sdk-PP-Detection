...
parity. The official PaddleDetection reference download has confirmed bytes and SHA-256,
but immutable Git LFS, Hugging Face, and ModelScope distribution copies plus complete
browser evidence are still pending, so it is not yet a stable SDK default. Pass a verified
runtime or custom manifest when using the SDK.
