# PP-Detection Web SDK Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build, validate, document, and publicly release a browser-first PP-Detection SDK with automatic WebGPU/WASM execution and versioned ONNX model assets.
...
The implementation lives in `F:\git\00_chenmohan\github\web-sdk-PP-Detection` after Task 1.
...
│  │  ├─ postprocess.ts               PP-Detection output decoding
│  │  ├─ preprocess.ts                800x800 tensor creation
│  │  └─ types.ts                     Public contracts
│  └─ tests/                          Unit and integration tests
├─ models/
│  ├─ pp-detection/1.0.0/manifest.json
│  └─ pp-detection/1.0.0/*.onnx     Git LFS objects
├─ tools/model-pipeline/
│  ├─ ppdetection/                    Python conversion library
...
**Files:**
- Existing: `docs/superpowers/specs/2026-08-11-web-sdk-pp-detection-design.md`
- Existing: `docs/superpowers/plans/2026-08-11-web-sdk-pp-detection.md`
...
gh auth status
npm whoami
gh repo view chenmohan123/web-sdk-PP-Detection --json name,url,visibility
npm view web-sdk-pp-detection name --json
```

Expected: GitHub and npm report the intended user. A missing GitHub repository and npm `E404` mean the preferred names are available. If the npm name exists, use `@chenmohan123/pp-detection-v3` for all later package metadata.
...
```powershell
gh repo create chenmohan123/web-sdk-PP-Detection --public --source . --remote origin --push
git branch main develop
git push -u origin main
gh repo edit chenmohan123/web-sdk-PP-Detection --default-branch main
```
...
git clone --branch develop https://github.com/chenmohan123/web-sdk-PP-Detection.git 'F:\git\00_chenmohan\github\web-sdk-PP-Detection'
git -C 'F:\git\00_chenmohan\github\web-sdk-PP-Detection' status --short --branch
```
...
describe("public package", () => {
  it("exports the factory and error type", () => {
    expect(sdk).toHaveProperty("createPPDetection");
    expect(sdk).toHaveProperty("PPDetectionError");
  });
});
```
...
Expected: FAIL because `createPPDetection` and `PPDetectionError` are not exported.
...
export async function createPPDetection(): Promise<never> {
  throw new PPDetectionError("SDK implementation is not initialized");
}
```
...
- Create: `tools/model-pipeline/ppdetection/inspect_model.py`
...
**Files:**
- Create: `tools/model-pipeline/ppdetection/export_fp32.py`
- Create: `tools/model-pipeline/ppdetection/export_wrapper.py`
- Create: `tools/model-pipeline/ppdetection/onnx_checks.py`
- Create: `tools/model-pipeline/tests/test_export_fp32.py`
- Create: `models/pp-detection/1.0.0/model-fp32.onnx`
...
```py
class PPDetectionExportWrapper(torch.nn.Module):
...
Run: `git lfs track "models/**/*.onnx"` and verify `git check-attr filter -- models/pp-detection/1.0.0/model-fp32.onnx` returns `lfs`.
...
**Files:**
- Create: `tools/model-pipeline/ppdetection/postprocess_reference.py`
- Create: `tools/model-pipeline/ppdetection/validate.py`
...
Compare Transformers vs ONNX for every fixture. Also run the Paddle model from `E:\models\PP-Detection` on the same images when its official runtime supports the current Python/Windows environment; record Paddle as an additional reference, not as a blocker when its runtime cannot be installed.
...
**Files:**
- Create: `tools/model-pipeline/ppdetection/convert_fp16.py`
- Create: `tools/model-pipeline/ppdetection/quantize_int8.py`
- Create: `tools/model-pipeline/tests/test_variants.py`
- Create: `models/pp-detection/1.0.0/model-fp16.onnx`
- Conditionally create: `models/pp-detection/1.0.0/model-int8.onnx`
...
**Files:**
- Create: `tools/model-pipeline/ppdetection/build_manifest.py`
...
Test valid default manifest parsing, unknown schema rejection, missing output rejection, SHA format rejection, duplicate precision/backend rejection, SDK minimum-version rejection, and serialization of `PPDetectionError` fields.
...
export class PPDetectionError extends Error {
  ...
  ) { super(message, options); this.name = "PPDetectionError"; }
}
```
...
Read `Response.body` chunks, publish monotonically increasing progress, combine once, compute SHA-256 with `crypto.subtle`, and reject before session creation on mismatch. Preserve the original cause in `PPDetectionError`.

- [ ] **Step 3: Implement cache keys and adapters**

The key is `ppdetection:<model-name>:<version>:<variant-id>:<sha256>`. Cache entries whose hash does not match the current manifest are never reused.
...
### Task 12: Implement PP-Detection Postprocessing
...
Export `createPPDetection`, `PPDetectionDetector`, manifest/result/options types, `PPDetectionError`, `probePPDetectionCapabilities`, and cache helpers. Keep internal ORT and cache adapters injectable through a non-public test seam rather than public configuration.
...
Commit: `feat(sdk): expose PP-Detection detector API`
...
Pack the SDK, install it into a temporary Vite fixture, and test ESM import, TypeScript declarations, browser global `PPDetection`, worker asset resolution, and absence of `.onnx` files in the npm tarball.
...
Use a tiny deterministic ONNX model for CI lifecycle tests, then run a tagged local suite against the real PP-Detection assets. Verify nonblank output, detection overlay coordinates, WASM, WebGPU when available, and model cache behavior.
...
Each example performs the same flow: create detector, show progress, select one image, render JSON or an overlay, handle a structured error, and dispose. CDN example uses `window.PPDetection`; React and Vue dispose on component unmount.
...
```powershell
npm view web-sdk-pp-detection version dist.integrity --json
npm install web-sdk-pp-detection@1.0.0
gh release view v1.0.0
```

Use `@chenmohan123/pp-detection-v3` in these commands if Task 1 selected the scoped fallback.
