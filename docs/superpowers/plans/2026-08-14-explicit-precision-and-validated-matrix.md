...
```ts
    expect(
    ...
    ).toEqual([
      {
        ...
        id: "fp16",
        precision: "fp16",
        url: "https://github.com/chenmohan123/web-sdk-PP-Detection/releases/download/v1.0.0-models/model-fp16.onnx"
      },
      {
        backendCompatibility: ["wasm"],
        id: "fp32",
        precision: "fp32",
        url: "https://github.com/chenmohan123/web-sdk-PP-Detection/releases/download/v1.0.0-models/model-fp32.onnx"
      }
    ]);
```
...
```powershell
pnpm --filter web-sdk-pp-detection exec vitest run tests/manifest.test.ts tests/runtime-selector.test.ts tests/detector.test.ts
```
...
Change only the FP32 variant coverage in `models/pp-detection/1.0.0/manifest.json`:
...
```powershell
git add -- models/pp-detection/1.0.0/manifest.json packages/sdk/tests/manifest.test.ts packages/sdk/tests/runtime-selector.test.ts packages/sdk/tests/detector.test.ts
...
```ts
import type { PPDetectionFallback } from "web-sdk-pp-detection";
...
export function formatFallbackCause(
  fallback: Pick<PPDetectionFallback, "cause" | "message">
): string {
...
- pass `allowFallback: allowFallbackForSelection(backend, precision)` to `createPPDetection`;
