...
**Goal:** Publish an immutable PP-Detection `1.0.1` model whose FP32 graph runs strictly on WebGPU and WASM, then adopt it as the default model in SDK `1.0.5` and the Demo.
...
- `tools/model-pipeline/ppdetection/sanitize_fp32.py`: guarded, deterministic DOUBLE-to-FLOAT transform for the four positional constants only.
...
- `tools/model-pipeline/ppdetection/validate.py`: accepted-versus-candidate FP32 raw-output and seven-fixture parity evidence.
...
- `tools/model-pipeline/ppdetection/build_manifest.py`: version/release parameters and browser-evidence gates.
- `tools/model-pipeline/tests/test_manifest.py`: versioned generation and fail-closed evidence tests.
- `models/pp-detection/1.0.1/`: sanitized FP32, byte-identical accepted FP16, and generated manifest.
...
- Create later: `tools/model-pipeline/ppdetection/sanitize_fp32.py`
...
from ppdetection.sanitize_fp32 import (
    POSITIONAL_NAMES,
    _double_names,
...
Expected: collection fails with `ModuleNotFoundError: No module named 'ppdetection.sanitize_fp32'`.
...
**Files:**
- Create: `tools/model-pipeline/ppdetection/sanitize_fp32.py`
- Modify: `tools/model-pipeline/tests/test_sanitize_fp32.py`
- Create: `models/pp-detection/1.0.1/model-fp32.onnx`
- Create: `models/pp-detection/1.0.1/model-fp16.onnx`

- [ ] **Step 1: Implement the fail-closed transform**

Create `tools/model-pipeline/ppdetection/sanitize_fp32.py` with this public contract and guarded transform:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Sanitize PP-Detection FP32 for WebGPU")
...
SOURCE_FP32 = ROOT / "models" / "pp-detection" / "1.0.0" / "model-fp32.onnx"
...
```powershell
.\.venv-model\Scripts\python.exe -m ppdetection.sanitize_fp32 --source models/pp-detection/1.0.0/model-fp32.onnx --output models/pp-detection/1.0.1/model-fp32.onnx
New-Item -ItemType Directory -Force models/pp-detection/1.0.1 | Out-Null
Copy-Item -LiteralPath models/pp-detection/1.0.0/model-fp16.onnx -Destination models/pp-detection/1.0.1/model-fp16.onnx
```
...
```powershell
$regen = Join-Path $env:TEMP 'ppdetection-model-fp32-1.0.1.onnx'
.\.venv-model\Scripts\python.exe -m ppdetection.sanitize_fp32 --source models/pp-detection/1.0.0/model-fp32.onnx --output $regen
Get-FileHash -Algorithm SHA256 models/pp-detection/1.0.1/model-fp32.onnx,$regen
Get-FileHash -Algorithm SHA256 models/pp-detection/1.0.0/model-fp16.onnx,models/pp-detection/1.0.1/model-fp16.onnx
...
```powershell
git add -- tools/model-pipeline/ppdetection/sanitize_fp32.py tools/model-pipeline/tests/test_sanitize_fp32.py models/pp-detection/1.0.1/model-fp32.onnx models/pp-detection/1.0.1/model-fp16.onnx
...
**Files:**
- Modify: `tools/model-pipeline/ppdetection/validate.py`
...
```python
from ppdetection.validate import canonical_json, sha256_file, validate_fp32, write_report
```
...
def test_sanitized_fp32_matches_accepted_fp32_and_official_transformers() -> None:
    accepted = ROOT / "models" / "pp-detection" / "1.0.0" / "model-fp32.onnx"
    candidate = ROOT / "models" / "pp-detection" / "1.0.1" / "model-fp32.onnx"
    report = validate_fp32(
        model_path=Path(r"E:\models\PP-Detection_safetensors"),
    ...
    assert report["sourceHashes"] == {
        ...
        "modelSafetensors": sha256_file(
            Path(r"E:\models\PP-Detection_safetensors\model.safetensors")
        ),
...
In `tools/model-pipeline/ppdetection/validate.py`:
...
```powershell
.\.venv-model\Scripts\python.exe -m ppdetection.validate --model E:\models\PP-Detection_safetensors --accepted-onnx models/pp-detection/1.0.0/model-fp32.onnx --onnx models/pp-detection/1.0.1/model-fp32.onnx --fixtures-lock tools/model-pipeline/fixtures/fixtures.lock.json --output tools/model-pipeline/reports/1.0.1/fp32-validation.json
```
...
```powershell
git add -- tools/model-pipeline/ppdetection/validate.py tools/model-pipeline/tests/test_parity_fp32.py tools/model-pipeline/reports/1.0.1/fp32-validation.json
...
- set `modelRoot` to `models/pp-detection/1.0.1` when the mode is `wasm-fp32` or `webgpu-fp32`;
...
**Files:**
- Modify: `tools/model-pipeline/ppdetection/build_manifest.py`
- Modify: `tools/model-pipeline/tests/test_manifest.py`
- Modify: `tools/model-pipeline/ppdetection/variant_validation.py`
...
- Create after generation: `models/pp-detection/1.0.1/manifest.json`
...
MODEL_DIR = ROOT / "models" / "pp-detection" / MODEL_VERSION
...
```powershell
.\.venv-model\Scripts\python.exe -m pytest tools/model-pipeline/tests/test_manifest.py tools/model-pipeline/tests/test_variants.py -q
git add -- tools/model-pipeline/ppdetection/build_manifest.py tools/model-pipeline/ppdetection/variant_validation.py tools/model-pipeline/tests/test_manifest.py tools/model-pipeline/tests/test_variants.py tools/model-pipeline/reports/1.0.1/variant-validation.json models/pp-detection/1.0.1/manifest.json
...
In `scripts/verify-release.mjs`, parse `--models <version>`, validate `/^\d+\.\d+\.\d+$/`, load `models/pp-detection/<version>/manifest.json`, and derive reports from `tools/model-pipeline/reports/<version>/`. For `1.0.1`, additionally require:
      ...
      - name: Create immutable model release
        ...
        run: |
          ...
          gh release create "${RELEASE_TAG}" \
            "models/pp-detection/${MODEL_VERSION}/manifest.json" \
            "models/pp-detection/${MODEL_VERSION}/model-fp16.onnx" \
            "models/pp-detection/${MODEL_VERSION}/model-fp32.onnx" \
            ...
            --target main \
            --title "PP-Detection model ${MODEL_VERSION}" \
            --notes "Immutable PP-Detection ${MODEL_VERSION} browser model assets."
        env:
...
$urls = @(
  'https://github.com/chenmohan123/web-sdk-PP-Detection/releases/download/v1.0.1-models/manifest.json',
  'https://github.com/chenmohan123/web-sdk-PP-Detection/releases/download/v1.0.1-models/model-fp16.onnx',
  'https://github.com/chenmohan123/web-sdk-PP-Detection/releases/download/v1.0.1-models/model-fp32.onnx'
)
...
Change SDK test fixtures to load `models/pp-detection/1.0.1/manifest.json`. In `detector.test.ts`, expect:

```ts
expect(DEFAULT_MANIFEST_URL).toBe(
  "https://chenmohan123.github.io/web-sdk-PP-Detection/models/v1.0.1/manifest.json"
);
```
...
expect(manifest.variants.find(({ id }) => id === "fp32")).toMatchObject({
  backendCompatibility: ["wasm", "webgpu"],
  precision: "fp32",
  url: "https://github.com/chenmohan123/web-sdk-PP-Detection/releases/download/v1.0.1-models/model-fp32.onnx"
});
```
...
```powershell
pnpm --filter web-sdk-pp-detection exec vitest run tests/detector.test.ts tests/manifest.test.ts tests/runtime-selector.test.ts
```
...
pnpm --filter demo exec vite build --base /web-sdk-PP-Detection/
...
```powershell
pnpm --filter web-sdk-pp-detection exec vitest run tests/manifest.test.ts
pnpm release:test
```
...
```powershell
pnpm --filter web-sdk-pp-detection pack --pack-destination test-results/package
...
```powershell
node scripts/verify-release.mjs --release v1.0.5
pnpm --filter web-sdk-pp-detection publish --dry-run
```

Only after a separate explicit confirmation, create and push tag `v1.0.5`. The existing release workflow must publish `web-sdk-pp-detection@1.0.5` through npm Trusted Publishing with provenance.
...
```powershell
npm view web-sdk-pp-detection@1.0.5 version dist.integrity dist.tarball --json
