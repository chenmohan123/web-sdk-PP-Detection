...
- Regenerate `packages/sdk/etc/web-sdk-pp-detection.api.md`: record the additive public type.
...
it("does not resurrect class candidates outside the global top-k", () => {
  const outputs: PPDetectionRawOutputs = {
...
```bash
pnpm --filter web-sdk-pp-detection exec vitest run tests/postprocess.test.ts
```
...
it("forwards class thresholds to the inference executor", async () => {
  ...
  const detector = await createPPDetectionWithDependencies(
    {},
...
Expected: FAIL because `PPDetectionDetectOptions` does not contain `classThresholds`.
...
```ts
export interface PPDetectionDetectOptions {
  readonly classThresholds?: Readonly<Record<string, number>>;
...
```bash
pnpm --filter web-sdk-pp-detection exec vitest run tests/detector.test.ts
pnpm --filter web-sdk-pp-detection typecheck
```
...
```bash
pnpm --filter web-sdk-pp-detection exec vitest run tests/worker-bridge.test.ts
```
...
```bash
pnpm --filter web-sdk-pp-detection exec vitest run tests/worker-bridge.test.ts tests/postprocess.test.ts tests/detector.test.ts
pnpm --filter web-sdk-pp-detection typecheck
```
...
```bash
pnpm --filter @ppdetection/demo exec playwright test tests/demo.spec.ts --grep "class threshold|Class threshold"
```
...
```bash
pnpm --filter @ppdetection/demo exec playwright test tests/demo.spec.ts
...
- Regenerate: `packages/sdk/etc/web-sdk-pp-detection.api.md`
...
Expected: build exits with code 0 and `packages/sdk/etc/web-sdk-pp-detection.api.md` shows `readonly classThresholds?: Readonly<Record<string, number>>;` in `PPDetectionDetectOptions`.
...
```bash
pnpm docs:test
pnpm --filter web-sdk-pp-detection api:report
```
...
```bash
git add README.md README.en.md packages/sdk/README.md docs/en/api.md docs/zh-CN/api.md docs/en/quick-start.md docs/zh-CN/quick-start.md scripts/check-doc-parity.test.mjs packages/sdk/etc/web-sdk-pp-detection.api.md
...
pnpm --filter @ppdetection/demo test
pnpm docs:test
```
