...
创建公开项目 `chenmohan123/web-sdk-PP-Detection`，提供基于 ONNX Runtime Web 的 PP-Detection 浏览器端版面检测 SDK，并发布到 npm。
...
以 `E:\models\PP-Detection_safetensors` 为主要转换源，以 `E:\models\PP-Detection` 的 Paddle 推理模型和官方 Transformers 推理结果为参考基准。
...
1. `createPPDetection` 读取默认或自定义 manifest。
...
远程仓库：`https://github.com/chenmohan123/web-sdk-PP-Detection`。

最终本地目录：`F:\git\00_chenmohan\github\web-sdk-PP-Detection`。
...
首个 npm 名称尝试 `web-sdk-pp-detection`；如被占用则使用 `@chenmohan123/pp-detection-v3`。
