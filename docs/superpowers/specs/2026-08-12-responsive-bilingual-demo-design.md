# PP-Detection Responsive Bilingual Demo
...
The demo calls `createPPDetection` and `detector.detect` through public exports only. Backend values are `auto`, `webgpu`, and `wasm`; precision values are `auto`, `fp16`, and `fp32`. Progress events are rendered as loading state, cancellation uses `AbortController`, structured `PPDetectionError` values are shown without exposing document pixels, and detector instances are disposed on replacement/unmount.
