...
Release `web-sdk-pp-detection@1.0.1` as a release-infrastructure and demo-reliability update. The release must prove npm Trusted Publishing works without npm tokens, make the npm package documentation useful, and make the public GitHub Pages demo load the official FP16 and FP32 models successfully in browsers.
...
- Configure npm Trusted Publishing for repository `chenmohan123/web-sdk-PP-Detection`, workflow `release.yml`, and GitHub environment `npm`.
...
GitHub About uses the Pages URL and an English description suitable for repository discovery. Topics cover PP-Detection, ONNX Runtime Web, document layout analysis, WebGPU, WASM, TypeScript, PaddleOCR, and OCR.
