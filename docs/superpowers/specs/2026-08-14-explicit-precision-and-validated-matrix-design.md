...
Fallback rows will include the most specific available cause text in addition to provider, precision, code, and stage. When a strict manual request fails, the main error message will include `PPDetectionError.details.causeMessage` when present. This exposes the ONNX Runtime or adapter reason without changing the public SDK error or fallback types.
