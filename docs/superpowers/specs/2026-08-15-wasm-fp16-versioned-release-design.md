...
Allow the bundled PP-Detection FP16 graph to run through ONNX Runtime Web WASM, while preserving the immutable published `1.0.0` model contract and making the new support reachable from the zero-configuration SDK and Demo.

## Versioning

- Keep `models/pp-detection/1.0.0/manifest.json` byte-for-byte compatible with the published `v1.0.0-models` release: FP16 supports WebGPU and FP32 supports WASM.
- Add `models/pp-detection/1.0.1/manifest.json`. It declares FP16 compatible with WASM and WebGPU, and FP32 compatible with WASM.
