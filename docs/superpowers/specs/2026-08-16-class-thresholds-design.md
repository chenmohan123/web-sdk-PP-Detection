...
Extend `PPDetectionDetectOptions` with a readonly class-name map:
...
The effective confidence threshold for a candidate is `classThresholds[label] ?? threshold ?? 0.5`. A class override applies to every label ID with that exact label name. This is intentional because the default PP-Detection manifest contains duplicate `formula`, `footer`, `header`, and `text` labels.
...
1. `PPDetectionDetector.detect()` accepts `classThresholds` in the public options.
