...
> 2026-09-16：当前 SDK 共 14 个规格、39 个稳定变体。Tiny 0.1.1 新增达标 FP16；W8A32 保留 labs。SDK/npm 保持 0.4.0。

[English](../en/models.md)

## 当前稳定清单（2026-09-16）

2026-09-16：当前 SDK 共 14 个规格、39 个稳定变体。Tiny 0.1.1 新增达标 FP16；W8A32 保留 labs。默认 FP32 和 ModelScope，允许显式选择 Hugging Face；未通过识别门槛的精度不进入稳定清单。
...
- PP-YOLO Tiny 清单：[`models/ppyolo-tiny-320/0.1.1/manifest.json`](../../models/ppyolo-tiny-320/0.1.1/manifest.json)
...
39 个变体均声明 WASM 与 WebGPU 后端。实际加载仍需浏览器能力探测，显式选择不兼容组合会返回 `CAPABILITY_UNSUPPORTED`，不会自动替换精度。`backend: "auto"` 只有配合 `allowFallback: true` 才能在有效候选的会话或推理失败后尝试下一后端。
...
Tiny 0.1.1 复用 0.1.0 FP32 固定来源并新增 2.36 MB FP16。FP16 最差 AP 变化 -0.174649 点、最低一对一检测保留率 99.5192%，通过固定 64 图双后端三轮门槛；W8A32 最差 AP 变化 -0.537364 点，保持 labs，未上传且不在 Demo 中开放。详见[质量报告](../../reports/evaluation/2026-09-16-tiny-precision/README.md)和[分发验证](../../reports/distribution/2026-09-16-tiny-precision/README.md)。
