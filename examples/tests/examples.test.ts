...
beforeAll(() => {
  sandbox = mkdtempSync(join(tmpdir(), "ppdetection-examples-"));
...
describe("consumer example content", () => {
  ...
  it("loads the CDN build through window.PPDetection", () => {
    const html = readExample("cdn", "index.html");
    expect(html).toMatch(/browser-global\.js/u);
    expect(html).toMatch(/window\.PPDetection/u);
