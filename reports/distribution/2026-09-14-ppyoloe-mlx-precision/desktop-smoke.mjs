...
const quality = JSON.parse(await readFile(resolve(root,
  "reports/evaluation/2026-09-14-ppyoloe-mlx-release/summary.json"), "utf8"));
assert.equal(quality.qualityGatePassed, true);
assert.equal(quality.browserRunCount, 54);
...
// 使用质量评估时的相同权重和实验清单，发布清单另由线上 Demo 验证。
const jobs = JSON.parse(await readFile(resolve(root,
  "reports/evaluation/2026-09-14-ppyoloe-mlx-precision/jobs.json"), "utf8"))
  .filter((job) => job.precision !== "fp32");
for (const job of jobs)
  routes.set(`/model/${job.size}-${job.precision}.onnx`, resolve(root, job.model));
...
try {
  for (const job of jobs) {
    ...
    const artifacts = {
      ...
      manifest: { path: job.manifest, sha256: createHash("sha256").update(manifestBytes).digest("hex") }
    };
    for (const backend of ["wasm", "webgpu"]) {
      ...
      for (const executionMode of ["main", "worker"]) {
        ...
        if (backend === "webgpu")
          assert.deepEqual(row.adapterIdentity, quality.webgpuAdapter.identity);
    ...
    JSON.stringify(
      {
        ...
        scope: "六个变体 × 两后端 × main/worker；预取消、恢复检测和 dispose，不覆盖全部进行中取消竞态",
        sdkSha256: createHash("sha256").update(await readFile(resolve(root, "packages/sdk/dist/browser-global.js"))).digest("hex"),
        protocolSha256: createHash("sha256").update(await readFile(resolve(root, "reports/evaluation/2026-09-14-ppyoloe-mlx-release/protocol.json"))).digest("hex"),
