...
assert.equal(manifest.model.version, "0.1.1");
assert.equal(manifest.status, "stable");
assert.deepEqual(manifest.variants.map(({ id }) => id), ["fp32", "fp16"]);
const variant = manifest.variants.find(({ id }) => id === "fp16");
assert.equal(variant.precision, "fp16");
assert.equal(variant.bytes, 2357376);
assert.equal(variant.sha256, "331cef176e8af2eacd9bfc6d011ade2d29cd41f013af2db2c716566e035413cc");
assert.deepEqual(variant.sources.map(({ kind }) => kind).sort(), ["huggingface", "modelscope"]);
...
const result = {
  ...
  status: "running",
  scope:
    "FP16双源×CPU/GPU×main/Worker：真实下载与SHA-256、预取消/恢复、释放/重复释放、缓存命中和两种清理；不覆盖进行中取消竞态、手机或NPU",
  sdkSha256,
...
const server = createServer(async (req, res) => {
  ...
  if (req.url === "/") {
    ...
    res.end(
      '<!doctype html><html lang="zh-CN"><meta charset="utf-8"><title>Tiny FP16分发验证</title><script src="/dist/browser-global.js"></script></html>'
    );
    return;
  }
    ...
    for (const backend of ["wasm", "webgpu"]) {
      for (const executionMode of ["main", "worker"]) {
        ...
        try {
          ...
          if (backend === "webgpu") assert(classifyAdapter(row.adapterIdentity).physical, "必须使用物理GPU");
          ...
          assert(row.detections.some(({ label }) => label === "person"));
