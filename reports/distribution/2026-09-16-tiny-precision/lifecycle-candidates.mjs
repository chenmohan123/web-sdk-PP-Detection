...
const report = resolve(process.cwd(), "reports/evaluation/2026-09-16-tiny-precision");
...
const jobs = JSON.parse(await readFile(jobPath, "utf8")).filter((j) => j.precision === "fp16");
const outName = resolve(process.cwd(), "reports/distribution/2026-09-16-tiny-precision/lifecycle-candidates.json");
      ...
      for (const executionMode of ["main", "worker"]) {
        ...
        const row = await page.evaluate(
          async ({ manifest, variant, key, backend, executionMode, origin }) => {
            ...
            await detector.dispose(); await detector.dispose();
            let disposedCode;
            try {
