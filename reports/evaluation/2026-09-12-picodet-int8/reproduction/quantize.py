...
for name,fmt,act in [('qdq-s8s8',QuantFormat.QDQ,QuantType.QInt8),('qoperator-u8s8',QuantFormat.QOperator,QuantType.QUInt8)]:
    target=OUT/'models'/f'{name}.onnx';start=time.perf_counter()
    try:
        if target.exists():raise FileExistsError(str(target))
        quantize_static(prepared,target,Reader(),quant_format=fmt,op_types_to_quantize=['Conv'],per_channel=True,activation_type=act,weight_type=QuantType.QInt8,calibrate_method=CalibrationMethod.MinMax)
        report['candidates'][name]={'status':'converted','method':fmt.name,'activation':act.name,'weight':'QInt8','perChannel':True,'quantizedOpTypes':['Conv'],'calibration':'MinMax','elapsedSeconds':time.perf_counter()-start,**evidence(target)}
