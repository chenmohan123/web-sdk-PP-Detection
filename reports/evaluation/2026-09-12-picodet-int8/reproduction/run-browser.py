...
for backend in ['wasm','webgpu']:
 for name in ['fp32','qdq-stem-fp32','qoperator-stem-fp32']:
    key=f'{backend}-{name}-1';target=OUT/'browser'/f'{key}.json';model=ROOT/'models/pp-detection/picodet-l-320-fp32.onnx' if name=='fp32' else OUT/'models'/f'{name}.onnx'
    ...
    (OUT/'browser/summary.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf-8')
