...
try {
  ...
  result.initialization=await page.evaluate(async({backend,precision,manifest,origin})=>{
  ...
  },{backend,precision:model.precision,manifest,origin});
  ...
  async function runImage(image,index){
    return page.evaluate(async({image,index})=>{
      ...
      return {imageId:image.id,fileName:image.file_name,width:raster.width,height:raster.height,order:index%2===0?'whole-first':'tiles-first',decodeMs,wholeMs,cropMs,tileDetectMs,projectMs,tileMergeMs,combinedMergeMs,modeMs:{whole:decodeMs+wholeMs,tiles:decodeMs+cropMs+tileDetectMs+projectMs+tileMergeMs,combined:decodeMs+wholeMs+cropMs+tileDetectMs+projectMs+combinedMergeMs},wholeTimings:whole.timings,tiles,whole:whole.detections,tiled,combined};
    },{image,index});
  }
  ...
  const suffix=limit===64?'':`-sanity${limit}`,file=join(inputs.outputs,`${name}-${variant}-${backend}${suffix}.json.gz`);
