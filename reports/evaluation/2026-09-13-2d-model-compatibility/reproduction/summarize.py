...
for backend in ['wasm','webgpu']:
    ...
    browser[backend]={'status':b['status'],'runtime':b['runtime'],'environment':b['environment'],'versions':b['runtimeVersions'],'performance':b['performance'],'comparison':{'reference':'Python ONNX Runtime + Pillow BICUBIC RGB /255','iouThreshold':.99,'scoreThreshold':.5,**aggregate(comparisons)},'evidence':f'evidence/browser-{backend}.json.gz'}
    stats=aggregate(comparisons)
    assert stats['unmatchedReferenceCount']==stats['unmatchedCandidateCount']==0
...
summary={'candidate':'ppyoloe-sod-l-640-coco','checkedAt':'2026-09-13','status':'selected','upstreamEvidence':'sources.lock.json','environment':read('raw-inspection.json')['environment'],'onnx':model_info,'rawOnnx':read('raw-inspection.json'),'preparation':read('preparation.json'),'pythonReference':{'imageCount':64,'annotationsSha256':paddle['annotationsSha256'],'iouThreshold':.99,'scoreThreshold':.5,'comparison':parity,'paddleMetrics':paddle['evaluation']['metrics'],'onnxMetrics':py['evaluation']['metrics'],'pillowMetrics':pillow['evaluation']['metrics'],'medianInferenceMs':py['medianInferenceMs'],'evidence':'evidence/paddle-reference.json.gz'},'browserSmoke':browser,'remaining':['完整 WASM/WebGPU × main/Worker 验证','取消、释放后调用、显式来源与后端失败路径的候选专项核验','至少一台移动设备的本模型验证','正式权重分发审查和来源锁定','独立小目标数据集与普通 PP-YOLOE+ L 的公平对照'], 'decision':'转换和 Python 参考核验通过，进入浏览器专项验证；暂不进入稳定模型清单。FP32 约为现有 PP-YOLOE+ S 的 10.8 倍；初步 smoke 不足以决定移动端接入。'}
