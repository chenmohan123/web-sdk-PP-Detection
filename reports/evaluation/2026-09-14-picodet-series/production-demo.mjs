...
const output = resolve(root, ".tmp/picodet-series-production");
...
const jobs = JSON.parse(await readFile(resolve(reportDirectory, "jobs.json"), "utf8"));
const picoKeys = [
  "xs-320",
  "xs-416",
  "s-320",
...
try {
  for (const key of picoKeys) {
    ...
    const variant = manifest.variants.find((item) => item.id === "fp32");
    ...
    try {
      ...
      if (key !== "picodet-l-320") {
        for (const name of ["FP16", "INT8"])
          assert(await precision.getByRole("button", { name, exact: true }).isDisabled());
      }
      await page
        .locator(".sample-card")
      ...
      for (const backend of ["CPU", "GPU"]) {
        ...
        assert.equal(result.model.variantId, "fp32");
        assert.equal(result.model.precision, "fp32");
        ...
        await writeFile(
          resolve(output, `${key}-${backend}.json`),
          JSON.stringify(result, null, 2) + "\n"
        );
        await page.screenshot({ path: resolve(output, `${key}-${backend}.png`), fullPage: true });
        await writeFile(
  ...
  assert.equal(report.manifests.length, 9);
