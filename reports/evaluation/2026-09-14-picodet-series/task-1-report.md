...
已完成清理与检查 API 的 `input_size` 参数化，支持 320、416、640，默认仍为 320，非法尺寸会明确拒绝；新增尺寸契约测试。新增可复现官方图下载/哈希准备脚本、来源锁定证据与 jobs 生成器，八个候选覆盖 XS/S/M 的 320、416 及 L 的 416、640，模型文件仅写入 `.tmp/picodet-series/`。
