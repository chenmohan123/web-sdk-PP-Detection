...
try {
  for (const deviceType of ["npu", "gpu"]) {
    ...
    rows.push(
      await page.evaluate(async (deviceType) => {
        const context = await navigator.ml.createContext({ deviceType });
        const builder = new MLGraphBuilder(context),
          descriptor = { dataType: "float32", shape: [4] };
  ...
  await writeFile(
    process.env.TEMP + "/ppdetection-webnn-control.json",
