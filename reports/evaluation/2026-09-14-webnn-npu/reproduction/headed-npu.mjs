...
const require = createRequire(
  path.join(process.env.PPDETECTION_REPO || process.cwd(), "package.json")
);
    ...
    for (const experimental of [false, true]) {
      ...
      try {
        browser = await chromium.launch({
          channel,
          headless: false,
          ignoreDefaultArgs: row.ignoredDefaultArgs,
          args
        });
        ...
        Object.assign(
          ...
          await page.evaluate(async () => {
            const result = {
              ...
              requestedDeviceType: "npu",
              actualPhysicalDevice: null
            };
              ...
              const operation = async () => {
                ...
                for (let index = 0; index < 5; index++) {
                  ...
                  result.runs.push({
                    ...
                    expected,
                    values,
                    passed: JSON.stringify(values) === JSON.stringify(expected)
                  });
                }
              };
              let timer;
              try {
                await Promise.race([
                  ...
                  new Promise((_, reject) => {
                    timer = setTimeout(() => reject(new Error("NPU 探针超时（20 秒）")), 20000);
                  })
                ]);
              } finally {
                clearTimeout(timer);
      ...
      try {
        row.nativeWebnnLog = (await readFile(logFile, "utf8"))
          .split(/\r?\n/)
          .filter((line) => line.includes("[WebNN]"))
          .map((line) => line.slice(line.indexOf("[WebNN]")));
      } catch {
        row.nativeWebnnLog = [];
      }
      rows.push(row);
...
const report = {
  ...
  purpose: "验证 API 请求和最小图；成功不代表实际物理 NPU",
  origin,
