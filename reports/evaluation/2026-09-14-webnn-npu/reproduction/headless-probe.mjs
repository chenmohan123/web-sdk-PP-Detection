...
const browsers = [
  ...
  { name: "Edge", executablePath: "C:/Program Files (x86)/Microsoft/Edge/Application/msedge.exe" },
  {
    name: "Chromium",
    executablePath:
      "F:/git/00_chenmohan/github/web-sdk-PP-Detection/.tmp/dependencies-compatible-browsers/chromium-1243/chrome-win64/chrome.exe"
  }
];
const server = createServer((req, res) => {
  res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
  res.end('<!doctype html><meta charset="utf-8"><title>WebNN NPU 本机验证</title>');
});
...
try {
  for (const item of browsers)
    for (const experimental of [false, true]) {
              ...
              try {
                ...
                context = await Promise.race([
                  ...
                  new Promise((_, reject) => {
                    timer = setTimeout(() => reject(new Error("上下文创建超时（20 秒）")), 20000);
                  })
                ]);
                clearTimeout(timer);
                entry.contextCreated = true;
                entry.limits =
                  typeof context.opSupportLimits === "function" ? context.opSupportLimits() : null;
                const builder = new MLGraphBuilder(context);
                const descriptor = { dataType: "float32", shape: [4] };
                const x = builder.input("x", descriptor);
                const y = builder.add(
                  x,
                  builder.constant(descriptor, new Float32Array([2, 3, 4, 5]))
                );
                ...
                entry.minimalGraph = {
                  ...
                  dataType: "float32",
                  values,
                  passed: JSON.stringify(values) === "[3,5,7,9]"
                };
              } catch (error) {
      ...
      rows.push(row);
      console.log(
        JSON.stringify({ ...row, contexts: row.contexts?.map(({ limits, ...rest }) => rest) })
      );
    }
} finally {
...
await writeFile(
  process.env.TEMP + "/ppdetection-webnn-probe.json",
