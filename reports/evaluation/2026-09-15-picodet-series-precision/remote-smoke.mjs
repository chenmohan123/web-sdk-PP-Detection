...
const jobs = JSON.parse(
  await readFile(
    resolve(root, "reports/evaluation/2026-09-15-picodet-series-precision/published-jobs.json"),
    "utf8"
  )
);
...
try {
    ...
    for (const sourceKind of ["modelscope", "huggingface"]) {
      ...
      for (const executionMode of ["main"]) {
        ...
        const row = await page.evaluate(
          async ({ manifest, variant, key, backend, executionMode, sourceKind, origin }) => {
          ...
          { manifest, variant, key, backend, executionMode, sourceKind, origin }
        );
        if (backend === "webgpu")
          assert.ok(
        ...
        assert.equal(
          row.model.source.revision,
          variant.sources.find((s) => s.kind === sourceKind).revision
        );
  ...
  assert.equal(rows.length, jobs.length * 2);
  await writeFile(
    ...
    JSON.stringify(
      {
        ...
        scope:
          "本轮新增稳定精度 × ModelScope/Hugging Face；最终清单、真实浏览器网络下载、SHA-256/CORS、WASM main 推理与释放",
