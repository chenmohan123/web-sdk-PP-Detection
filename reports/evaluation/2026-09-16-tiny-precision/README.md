...
独立审查后增加实际标注摘要/数量/图片ID、完整评测配置、有限且非负耗时以及三精度CPU输入输出契约验证，见 supplemental-validation.json。该补充不改变原运行协议、权重、预测或运行时间；18组归档复算与原summary完全一致。13项回归测试通过，三种模型CPU会话契约及有限输出再次核验通过。FP16可继续进入发布流程，W8A32仍为labs。
