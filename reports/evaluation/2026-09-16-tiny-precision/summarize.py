...
def quality_pass(delta,ret): return validation.finite_number(delta) and delta>=-.5 and validation.finite_number(ret) and .95<=ret<=1
def require(v,m):
...
def synthetic_record(job,lock,backend):
    ...
    return {"schemaVersion":1,"status":"passed","capturedAt":"2026-09-16T00:00:00Z","environment":lock["environment"],"runtimeVersions":lock["environment"]["runtimeVersions"],"runtime":{"requestedBackend":backend,"backend":backend,"mode":"main","precision":precision(job["precision"]),"fallbacks":[]},"model":{"id":ident["modelId"],"version":ident["modelVersion"],"variantId":job["precision"],"precision":precision(job["precision"]),"bytes":job["bytes"]},"evaluation":validation.expected_evaluation(backend,job["precision"]),"artifacts":{"model":{"sha256":job["sha256"],"bytes":job["bytes"]},"manifest":{"sha256":ident["manifestSha256"]},"sdk":{"sha256":lock["sdkSha256"]},"annotations":{"sha256":ANNOTATIONS_SHA},"imageCount":64,"imageSetSha256":lock["imageSetSha256"]},"images":[{**x,"wallClockMs":1.0,"timings":{"inferenceMs":1.0}} for x in lock["images"]],"predictions":[]}
def validate_record(v,binding,raw,job,backend,round_no,lock):
    validation.validate_evaluation(v,backend,job["precision"])
    validation.validate_timings(v)
...
def summarize(archive=False):
    ...
    if archive:
      lock=make_lock(image_root); (REPORT/"inputs.lock.json").write_text(json.dumps(lock,ensure_ascii=False,indent=2,allow_nan=False)+"\n",encoding="utf-8")
    else:
    ...
    annotations=validation.load_annotations(ANNOTATIONS,ANNOTATIONS_SHA,lock)
    validation.validate_cpu(read(REPORT/"cpu-validation.json"),jobs)
    ...
    result={"protocolSha256":lock["protocolSha256"],"sdkSha256":lock["sdkSha256"],"inputsLockSha256":sha((REPORT/"inputs.lock.json").read_bytes()),"environment":{**lock["environment"],"gpuAdapter":lock["gpuAdapter"]},"browserRunCount":len(rows),"qualityGate":protocol["qualityGate"],"candidates":candidates,"rows":rows,"qualityGatePassed":all(x["qualityGatePassed"] for x in candidates)}; (REPORT/"summary.json").write_text(json.dumps(result,ensure_ascii=False,indent=2,allow_nan=False)+"\n",encoding="utf-8")
    if archive:
      idx=[]
      for rd in (1,2,3):
       for j in jobs:
      ...
      (REPORT/"artifact-index.json").write_text(json.dumps(idx,ensure_ascii=False,indent=2,allow_nan=False)+"\n",encoding="utf-8")
    if (REPORT/"artifact-index.json").exists():
      bound=["protocol.json","jobs.json","inputs.lock.json","summary.json","artifact-index.json","cpu-validation.json","validation.py","supplemental-validation.json"]
      receipt={"files":{name:sha((REPORT/name).read_bytes()) for name in bound},"evidence":read(REPORT/"artifact-index.json"),"browserRunCount":len(rows),"annotationsSha256":ANNOTATIONS_SHA}
      (REPORT/"quality-receipt.json").write_text(json.dumps(receipt,ensure_ascii=False,indent=2,allow_nan=False)+"\n",encoding="utf-8")
    return result
