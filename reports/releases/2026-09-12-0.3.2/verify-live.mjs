...
const output = resolve(process.env.PPDETECTION_RELEASE_REPORT ?? ".tmp/release-0.3.2-live");
...
try {
  ...
  assert.equal(await page.locator(".version").innerText(), "SDK 0.3.2");
  ...
  for (const model of ["picodet-l-320", "ppyoloe-plus-s-640"]) {
    ...
    for (const backend of ["wasm", "webgpu"]) {
      ...
      assert.equal(result.model.version, model === "picodet-l-320" ? "1.0.2" : "0.1.1");
