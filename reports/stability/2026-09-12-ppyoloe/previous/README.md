# PP-YOLOE+ S 640 FP32（实验）

此目录描述 `ppyoloe-plus-s-640` 的 `0.1.0-labs.1` 固定版本。模型用于浏览器端目标检测，输入为 `1x3x640x640` FP32 张量，输出包含图内 NMS 后的 `[N,6]` 检测结果和 `[1]` 检测数量。该版本仍为 `labs`，不会替代 SDK 默认的稳定 PicoDet 模型。
...
- 该版本是 FP32 实验模型，仅验证 `wasm` 和 `webgpu`；FP16、INT8、INT4、FP8 未纳入本版本。
- 尚未完成移动设备、微信 WebView、其他浏览器和其他 GPU 的独立验证。
