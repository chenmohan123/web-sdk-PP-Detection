...
function compileSnippet(source, name) {
  ...
  const options = {
    ...
    noEmit: true,
    paths: {
      "web-sdk-pp-detection": ["packages/sdk/src/index.ts"]
    },
    skipLibCheck: true,
    strict: true,
...
assert(
  ...
  "docs/error-codes.json and PPDetectionErrorCode are out of sync"
);
assert(
...
for (const required of [
  ...
  assert(
    combinedReadmes.toLocaleLowerCase().includes(required.toLocaleLowerCase()),
    `README coverage is missing: ${required}`
  );
}
