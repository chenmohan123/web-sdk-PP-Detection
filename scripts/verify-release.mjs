...
function verifyStaticContract() {
  ...
  const expectedRepositoryUrl = "git+https://github.com/chenmohan123/web-sdk-PP-Detection.git";
  if (
  ...
  requireMatch(
    pages,
    /pnpm --filter web-sdk-pp-detection build[\s\S]*pnpm --filter demo exec vite build/,
  ...
  requireMatch(
    ...
    /labs\/blocked|isBlockedManifest/,
    "Pages model staging must disclose blocked model manifests"
  );
...
async function verifyModels(modelVersion) {
  ...
  if (manifest.status === "labs/blocked") {
    fail(`model ${modelVersion} is blocked and cannot be released`);
  }
