...
describe("发布工作流契约", () => {
  test("验证当前 0.2.0 SDK 的稳定模型清单静态配置", () => {
  ...
  test("package、runtime 和 changelog 版本保持 0.2.0 一致", () => {
    ...
    assert.match(read("sdk-manifest.yaml"), /version: 0\.2\.0/);
    assert.match(runtime, /CURRENT_SDK_VERSION = "0\.2\.0"/);
    assert.match(changelog, /^## 0\.2\.0（2026-09-08）$/m);
  });
