...
test.beforeAll(async () => {
  fixtureRoot = mkdtempSync(join(tmpdir(), "ppdetection-package-"));
  runPnpm(["--filter", "web-sdk-pp-detection", "build"], repositoryRoot);
  ...
  writeFileSync(
    ...
    JSON.stringify({
      dependencies: {
        "web-sdk-pp-detection": `file:${tarball.replaceAll("\\", "/")}`
      },
  ...
  installedSdkRoot = join(fixtureRoot, "node_modules/web-sdk-pp-detection");
...
test("packed ESM and declarations install in a standalone Vite project", () => {
  ...
  writeFileSync(
    ...
    [
      'import { createPPDetection, type PPDetectionResult } from "web-sdk-pp-detection";',
      "const result: PPDetectionResult | undefined = undefined;",
      'document.querySelector("#app")!.textContent = `${typeof createPPDetection}:${String(result)}`;'
    ].join("\n")
  );
  writeFileSync(
...
test("publishes a browser global and a resolvable worker asset", async ({ page }) => {
  ...
  await expect
    .poll(() => page.evaluate(() => typeof window.PPDetection?.createPPDetection))
    .toBe("function");
  const result = await page.evaluate(
    async ({ base64, manifest, wasmBaseUrl }) => {
      ...
      let detector;
      try {
        detector = await window.PPDetection!.createPPDetection({
          allowFallback: false,
          backend: "wasm",
          cache: false,
